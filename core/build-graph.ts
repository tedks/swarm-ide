...
export async function buildInputDigest(root: string, signal: AbortSignal): Promise<string | null> {
  ...
  for (const path of paths) {
    ...
    try { const stat = await lstat(join(root, path)); hash.update(stat.isFile() ? "file" : stat.isDirectory() ? "directory" : stat.isSymbolicLink() ? "link" : "special"); }
  ...
  try {
    ...
    return await new Promise<Uint8Array>((resolve, reject) => {
      ...
      const consume = (chunk: Uint8Array, retain: boolean) => {
        ...
        else if (retain && !closing) chunks.push(Buffer.from(chunk));
      };
      try {
...
export class BuildGraphProvider {
  ...
  observe(refresh = false): BuildGraphObservation {
    ...
    if (!this.pending && (refresh || this.dependencies.now() - this.checkedAt >= 1500 || !this.checkedAt)) {
      ...
      if (refresh || !this.state.graph) this.state = { ...this.state, status: "refreshing", message: "Observing Bazel declarations; not building binaries." };
      this.pending = this.reconcile(refresh).finally(() => { this.pending = undefined; });
    }
  ...
  async dispose(): Promise<void> { this.closed = true; this.controller.abort(); await this.pending; }
}
