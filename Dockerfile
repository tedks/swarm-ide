...
# Dependencies were frozen above; do not let each pnpm exec start another install.
RUN nix develop --option sandbox false --option filter-syscalls false --command bash -c 'bazel --batch build --jobs=3 --action_env=HOME=/root --action_env=CI=true --action_env=pnpm_config_pm_on_fail=ignore --action_env=pnpm_config_verify_deps_before_run=false //:desktop-bundle && mkdir -p /opt/swarm && tar -xzf bazel-bin/swarm-ide-foundation.tar.gz -C /opt/swarm'
