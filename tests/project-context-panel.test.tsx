...
it("shows real endpoints and compact missing containers without changing attention", async () => {
  ...
  expect(screen.getByText("No containers for this worktree")).toBeTruthy();
...
it("contains bridge failure as compact unavailable state", async () => {
  window.swarm = { request: async () => { throw new Error("Disconnected"); }, onEvent: () => () => {} } as unknown as Window["swarm"];
  render(<ProjectContextPanel repositoryId={snapshot.project.id} worldId={snapshot.world.id} generation={1} ready />);
  await waitFor(() => expect(screen.getByText("Runtime context unavailable")).toBeTruthy());
});
