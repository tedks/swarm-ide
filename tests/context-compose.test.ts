...
describe("exact Context composition", () => {
  it("distinguishes ordinary, implementation, provided and required declarations", async () => {
    for (const [path, labels] of [["core/files.ts", []], ["example/validator.ts", ["Implementation member of"]], ["example/validator.proto", ["Implementation member of", "Declares provided interface"]], ["example/writer.proto", ["Declares required interface"]]] as const) {
  ...
  it("preserves independent receipts and never treats unavailable, failed or old-core inputs as green", async () => {
    const { subject, input } = await setup("example/validator.ts");
  ...
  it("indexes exact capture references once with helper-equivalent semantics and bounded partial display", async () => {
    const { subject, input } = await setup("example/validator.ts");
    const capture = { repositoryId: subject.repositoryId, revision: "capture-a", command: "bazel query", capturedAt: "2026-09-06T01:00:00.000Z", links: Array.from({ length: 40 }, (_, i) => ({ from: `//example:target${i}`, fromPath: "example", to: "//example:validator.ts", toPath: "example/validator.ts" })) };
    input.capture = indexCapture(capture);
    expect(input.capture.paths.get("example/validator.ts")).toEqual(fileBuildTargets(capture, "example/validator.ts"));
