...
export const CoreRequestSchema = z.union([WorkspaceRequestSchema, AgentRequestSchema, TaskRequestSchema, RepositoryRequestSchema, RepositorySearchRequestSchema, ChangelogRequestSchema, PlanReadRequestSchema, ExternalRequestSchema, BuildGraphRequestSchema, TaskActivityRequestSchema]);
...
export function parseCoreResponseForRequest(input: unknown, request: CoreRequest): CoreResponse {
  ...
  if (request.type === "taskActivity.read") {
    if (response.ok) {
      ...
      if (!result || response.task || response.agent || response.file || response.repo || response.search || response.changelog || response.plans || response.external || response.buildGraph ||
