...
export const CoreRequestSchema = z.union([WorkspaceRequestSchema, AgentRequestSchema, TaskRequestSchema, RepositoryRequestSchema, RepositorySearchRequestSchema, ChangelogRequestSchema, BuildGraphRequestSchema]);
...
export function parseCoreResponseForRequest(input: unknown, request: CoreRequest): CoreResponse {
  ...
  if (request.type === "changelog.read") {
    if (response.ok && (!response.changelog || response.buildGraph || response.file || response.agent || response.task || response.repo || response.search ||
  ...
  if (request.type === "buildGraph.observe") {
    if (response.ok && (!response.buildGraph || response.changelog || response.file || response.agent || response.task || response.repo || response.search ||
