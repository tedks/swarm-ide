# Codex 0.153.4 limited launch profile: P1 investigation
...
Observed on 2026-09-06: the complete installed npm package is `@openai/codex`
0.153.4, with Linux x64 musl package layout version 1. The launcher is
`/home/tedks/.npm-global/bin/codex`; its package resolves under
`/home/tedks/.npm-global/lib/node_modules/@openai/codex/`. The platform package's
...
E1's archived generated schemas, and a disposable checkout of the package's
...
1. **Startup precedes inspection.** `codex-rs/app-server/src/lib.rs:504`
   ...
   config loader before client initialization. `message_processor.rs:369`
   starts the model refresh worker, and `:525` starts plugin warmups.
...
2. **An isolated user config is not the entire configuration boundary.**
   ...
   not exposed by that CLI path. The top-level profile option must not be
   assumed to isolate app-server: its invocation does not pass profile loader
   overrides. `--strict-config` rejects invalid configuration; it is not
...
4. **Aliases matter after merging.**
   ...
   `multi_agent`, and `codex_hooks` to `hooks`. `Features::apply_map` in
...
5. **The main hooks flag is not every hook.**
   ...
   *and* plugin sources are empty; at `:255`, built-in plugin handlers survive
   ...
   the resolved plugin feature is disabled. These are distinct gates.
...
6. **Config inspection is useful but not an atomic attestation.**
   ...
   list at `:65` does not include hooks/apps/plugins/multi_agent.
...
boundary that hides the real credential/config locations, supplies an explicit
non-secret environment, prevents network independently of Codex, and makes
fixture configuration immutable for the process lifetime. A separate mount
...
Within that boundary, use the actual complete 0.153.4 package and observe a
fresh stdio server without a turn. Include hostile user/project/managed-layer
fixtures, alias collisions, legacy notify, ordinary/bundled plugin hooks,
...
seam** before `turn/start`. Its evidence must bind executable/companion identity,
...
returned thread policy. Recheck after relevant changes; a generic earlier
