...
identities remain ambiguous. Present helper links are walked transitively and cycles
are rejected; only an absent intermediary ends that consistency check. Header
classification never substitutes for the existing
...
count cutoff; each accessible non-bare, non-prunable root gets separate repository
and exact-top-level probes, safely supporting newline-containing paths, in a bounded
worker pool under the existing cancellation and total deadline. The pool drains
before reporting cancellation. Explicit selection continues to choose the
