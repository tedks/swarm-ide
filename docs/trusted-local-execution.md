...
Raw provider stderr is withheld. The service/history keeps up to a 128KiB UTF-8 output tail and the last fifty activity entries; a per-record 256KiB serialized bound may omit additional older entries. Output truncation is labelled. The provider-activity accessor is optional until its separate implementation joins. Storage failures are visible and block new commands while preserving Stop authority. A failed session with unconfirmed cleanup continues occupying a live slot until the core restarts.
