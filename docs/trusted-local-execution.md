...
The local core supports eight simultaneous trusted conversations (including owners whose cleanup is still uncertain), with one context preparation at a time and twenty retained records. In the dock, **New agent** opens a fresh composer without stopping existing runs; the run list selects a conversation's output, activity, approvals and controls. Each run has its own message composer while the cockpit remains mounted. Unsent composers are not persisted across an app reload; submitted messages are saved before dispatch and remain copyable after reload. Each run has an immutable token and captured worktree: targeted snapshots, Send, approval and Stop affect that run regardless of current navigation. Task Context can open a conversation actually admitted with that task attached, including saved history; a finished turn does not close the task. The compatible no-token snapshot API still selects the latest run.
