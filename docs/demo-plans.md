...
The view reads at most 64 details, with four concurrent requests. Open tasks come before closed tasks, then IDs sort deterministically. It shows up to 128 additional endpoints and 512 unique edges. Coverage counts describe the complete snapshot denominator and the loaded subset; omitted declarations/endpoints and unread details stay visible. Isolated loaded tasks remain in the graph. Missing, cyclic and asymmetric declarations are shown literally. No known blockers is **not** dispatch authorization. Refreshing metadata does not silently substitute a new task revision beneath an old graph: explicitly load again. A changed core lifetime makes retained information non-current.
