...
`WorkbenchSidebar` accepts a Work Log slot below agent runs. Raw activity stays in the persistent Activity area, while `FleetActivityView` displays the fleet's timestamped entries in the center Activity log. Each event retains its containing session identity when opening source or a recorded patch. Live activity and saved summaries are separate tabs. F7 owns fleet observation; K7 owns Work Log production; P7 owns recursive component documents. C7 owns their application mounts and the shared theme. Work Log and design mounts are pending reviewed producer commits, not claimed from placeholders.
