  ...
  return (
    <section className={`graph-pane ${graph.directory ? "has-directory" : ""} ${explorer ? `repository-view-${repositoryView}` : ""}`} data-topology={graph.topologyId} data-directory={graph.directory?.directory} data-observation-state={graph.directory?.state}
      ...
      <header className="graph-header">
        <div title={onActivate ? "Click or Enter: declaration. Alt-click or Shift+Enter: inspect. Declarations are not callsites." : undefined}><span className="eyebrow">{explorer ? "repository" : `${graph.topologyId} lens`}{onActivate ? " · ↵ definition" : ""}</span>{!explorer ? <h2>{graph.title}</h2> : null}</div>
