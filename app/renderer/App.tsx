...
export function App() {
  ...
  const browseAgentWorktree = (sessionId: string) => {
    ++navigationIntent.current;
    setWorkLogEntry(null); setWorktreeSelection(null); setWorktreeBrowserSession(sessionId); setWorktreeVisible(true);
    setDesignVisible(false); setJournalVisible(false); setTaskDocumentVisible(false);
  };
  ...
  }, [activeSurface, closeFile, interruptPendingReveal, resetZoom, zoomIn, zoomOut, taskDocumentVisible, taskDocumentOpen, paletteOpen, cancelPalette, openPalette, closeTaskDocument, journalVisible, worktreeVisible, designVisible, overviewVisible, workLogEntry]);
  ...
  const textDocumentVisible = !workLogEntry && !journalVisible && !worktreeVisible && !designVisible && taskDocumentVisible;
  const textOpen = Boolean(!overviewVisible && activeFile || designVisible || textDocumentVisible || journalVisible || worktreeVisible || workLogEntry);
  const hasOpenDocument = Boolean(fileTabs.length || designVisible || taskDocumentOpen || journalOpen || worktreeSelection || worktreeBrowserSession || workLogEntry);
  ...
  return (
    <main className="workbench" onPointerDownCapture={interruptPendingReveal} onFocusCapture={interruptPendingReveal} onKeyDownCapture={(event) => { if (event.key === "Escape" && definitionRef.current) { event.preventDefault(); event.stopPropagation(); cancelDefinition(); } }} data-compact-panel={compactPanel ?? "none"} style={{ "--context-width": `${contextWidth}%`, ...(dockShare !== null ? { gridTemplateRows: `var(--topbar-height) minmax(0, 1fr) ${dockShare}%` } : agents.selected || liveAgents.paneOpen ? { gridTemplateRows: `var(--topbar-height) minmax(0, 1fr) calc(160px + (clamp(180px, 40vh, 448px) - 160px) * ${Math.min(1, Math.max(0, ((liveAgents.paneOpen ? liveAgents.height : agentPaneHeight) - 230) / 190))})` } : {}) } as CSSProperties}>
      <header className="topbar">
        ...
        <nav className="lens-tabs" aria-label="Workspace navigation"><button className={!textOpen ? "active" : ""} onClick={showOverview}>Workspace</button></nav>
      ...
      <section className={`navigation-field unified-graph-workspace ${textOpen ? "source-open" : ""}`} style={{ "--graph-share": `${graphShare}%` } as CSSProperties}>
        ...
        {hasOpenDocument ? <OverflowStrip className="surface-tabs-strip" label="document tabs" activeKey={workLogEntry?.id ?? (designVisible ? "design" : worktreeVisible ? `worktree:${worktreeSelection?.path}` : journalVisible ? "journal" : textDocumentVisible ? "task" : activeSurface)}><nav className="surface-tabs" aria-label="Document tabs">
        ...
        <PlanWorkspace key={`${snapshot.project.id}:${snapshot.world.id}`} visible worldId={snapshot.world.id} repositoryId={snapshot.project.id}
        ...
        {activeFile ? <section hidden={overviewVisible || textDocumentVisible || journalVisible || worktreeVisible || designVisible || Boolean(workLogEntry)} onPointerDown={sourceInformation} onFocusCapture={sourceInformation} className={`source-surface ${["conflict", "unknown", "error"].includes(activeFile.status) ? "has-banner" : ""}`}>
