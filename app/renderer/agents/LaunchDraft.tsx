...
export function LaunchDraft({ focus, onLaunch, onClose }: { focus: FocusRef; onLaunch: (context: LaunchContext) => void; onClose: () => void }) {
  ...
  const valid = Boolean(task.trim()) && utf8Bytes(task) <= AGENT_LIMITS.taskBytes && utf8Bytes(model) <= 256 && utf8Bytes(effort) <= 64 &&
    AgentFocusSchema.safeParse(launchFocus).success && launchFocus.revisionKind === "working";
  return <form className="agent-draft" onSubmit={(event) => { event.preventDefault(); if (valid) onLaunch(fixtureLaunchContext(launchFocus, task, model, effort)); }}>
    ...
    {!valid ? <p role="alert">Use a working-world focus and nonempty task within the UTF-8 limit.</p> : null}
    <button className="agent-primary" disabled={!valid}>Launch fixture — no provider</button>
  </form>;
}
