...
function operationMeaning(operation: LocalOperation): string {
  if (operation.status === "delivery-unknown") return "Delivery could not be confirmed. Check the conversation before sending again; no automatic retry will occur.";
  if (operation.status === "pending") return "Waiting for acknowledgement.";
  if (operation.status === "rejected") return "Command rejected. Your text is saved here and has not been resent.";
  if (operation.kind === "launch") return "Launch request accepted.";
  if (operation.kind === "cancel") return "Stop request accepted. Check the run status for confirmation.";
...
export function LiveRunPane({ state, onInstruction, onSteer, onStop, onRead, onFollow, onReveal, onClose, onHeight, currentFingerprint, currentWorldId }: {
  ...
  const stateLabel = run?.state ?? summary?.state ?? "waiting for run details";
  ...
  return <section className="agent-pane" aria-label="Selected agent run">
    ...
    <header className="agent-pane-header"><div><strong tabIndex={0}>{displayAgentText(summary?.taskLabel ?? "Selected agent run")}</strong><span className={`agent-state agent-state-${run?.state ?? summary?.state ?? "unknown"}`}>{stateLabel}</span></div><div>
      <button title="Open this run's source in the current working revision" disabled={!run} onClick={() => { if (run) onReveal(run.launchContext.focus); }}>Reveal launch focus</button>
    ...
    <div className="agent-pane-body"><div className="agent-output" tabIndex={0} aria-label="Agent output">
      <div className="agent-evidence">
        ...
        {run ? <div>Process cleanup: {displayAgentText(run.cleanup.detail)} · {run.cleanup.observedAt}{run.exitCode !== null ? ` · exit code ${run.exitCode}` : ""}</div> : null}
        {run ? <div>Result: {run.providerOutcome.kind}{run.providerOutcome.kind === "turn" ? ` · ${run.providerOutcome.status} · ${run.providerOutcome.observedAt}` : ""}</div> : null}
        {run?.terminalReason ? <div>Run ended: {displayAgentText(run.terminalReason)}</div> : null}
        {run?.state === "completed" && operations.some((operation) => operation.kind === "cancel" && operation.status === "accepted") ? <div>The turn completed despite the Stop request.</div> : null}
        {run?.cleanup.status === "unknown" ? <div>The process may still be running; cleanup could not be confirmed.</div> : null}
        {!fresh ? <div role="status">{state.connected ? "Run details are out of date or unavailable. Controls are disabled." : "Disconnected. Showing saved details; commands will not be resent."}</div> : null}
        {worldAdvanced ? <div>The workspace has changed since launch. Reveal opens the current source; submitted context stays unchanged.</div> : null}
      </div>
      <div className="agent-transcript" role="log" aria-label="Agent transcript" aria-live="polite" aria-relevant="additions text" aria-busy={state.reading}>
        {missingEarlierRecords ? <p className="agent-gap">Earlier messages are on another page. Choose Read transcript from start to see them.</p> : null}
        {state.pageTruncated || run?.transcript.truncated ? <p className="agent-gap">Some transcript records were omitted. Read earlier pages to see the available history.</p> : null}
        {run?.transcript.tailMayBeLost ? <p className="agent-gap">The last messages may have been lost during recovery.</p> : null}
        {state.records.length > AGENT_LIMITS.pageRecords ? <p className="agent-gap">This page has reached its display limit.</p> : null}
      ...
      {run ? <details><summary>Submitted context</summary><LaunchContextView context={run.launchContext} /></details> : <p>Waiting for run details. Your command history is below.</p>}
      ...
      <form onSubmit={(event) => { event.preventDefault(); if (steerEnabled) onSteer(); }}>
        ...
        <small>{instructionBytes} / {AGENT_LIMITS.taskBytes} UTF-8 bytes. Your text stays here after sending.</small>
        <div className="agent-steer-actions"><button disabled={!steerEnabled}>Send to this run</button></div>
      </form>
      {unsettledSteer ? <p>Waiting to confirm your last instruction. Check its status below before sending another.</p> : null}
      {unsettledStop ? <p>Stop is awaiting confirmation. The run may still be active; the request will not be resent.</p> : null}
      {stopRequested && !unsettledStop ? <p>Stop request acknowledged. Check the run and process status above.</p> : null}
      <p>You can send instructions while the turn is running.</p>
      <section className="agent-receipts" aria-label="Instruction receipts">
        ...
        {receipts.map((receipt) => <details key={receipt.requestId} open={unresolved(receipt.status)}><summary>Instruction · {receipt.status}</summary><pre>{displayAgentText(receipt.text)}</pre><p>{receipt.error ? `${receipt.error.code}: ${displayAgentText(receipt.error.message)}` : receipt.status === "accepted" ? "Instruction acknowledged." : receipt.status === "pending" ? "Waiting for acknowledgement." : "Delivery could not be confirmed. Check the conversation before sending again; no automatic retry will occur."}</p><small>Request: {displayAgentText(receipt.requestId)}<br />Expected turn: {displayAgentText(receipt.expectedTurnId)}<br />Submitted: {receipt.submittedAt}<br />Settled: {receipt.settledAt ?? "pending"}<br />Text hash: {receipt.textHash}</small></details>)}
