...
export function RunPane({ state, dispatch, onReveal, onClose, height, onHeight }: {
  ...
  return <section className="agent-pane" aria-label="Selected agent run">
    ...
    <header className="agent-pane-header"><div><span className="agent-demo-badge">FIXTURE / DEMO</span><strong tabIndex={0}>Focus analysis</strong><span className={`agent-state agent-state-${run.state}`}>{run.state}</span></div><div><button title="Reveal in the current working revision, which may have advanced since launch" onClick={() => onReveal(run.launchContext.focus)}>Reveal launch focus</button><button onClick={() => dispatch({ type: "stop" })} disabled={run.state !== "running" && run.state !== "starting"}>Stop</button><button aria-label="Close run pane" onClick={onClose}>×</button></div></header>
    <div className="agent-pane-body"><div className="agent-output" tabIndex={0} aria-label="Agent output">
      ...
      <details><summary>Submitted context</summary><p>{run.launchContext.focus.path ?? run.launchContext.focus.key}</p><p>Disk only, no file bytes attached. Requested model: {run.launchContext.requested.model ?? "provider default (unresolved)"}; reasoning: {run.launchContext.requested.effort ?? "provider default (unresolved)"}. Root, digests and timestamps are synthetic. No verified policy or measured usage. Reveal maps the launch focus to the current working revision.</p><pre>{run.launchContext.submittedPrompt}</pre></details>
