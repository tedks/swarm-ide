...
function BuildNode({ data }: NodeProps) {
  return <div className={`build-target-node${data.ownsFile ? " build-target-file-owner" : ""}`}><Handle type="target" position={Position.Left} /><small>{data.ownsFile ? "REFERENCES FILE · SNAPSHOT" : "BAZEL TARGET · SNAPSHOT"}</small><strong>{String(data.label)}</strong><Handle type="source" position={Position.Right} />{data.mock ? <AgentSprites count={1} /> : null}</div>;
}
const nodeTypes = { buildTarget: BuildNode };

export function TopologyViews({ service, capture, mockAgents, mockVersion, onOpenBuild, reframeVersion = 0, showBuildVersion = 0, focusedFile = null }: { service: ReactNode; capture?: BuildLinkSnapshot; mockAgents: boolean; mockVersion: number; onOpenBuild: (path: string) => void; reframeVersion?: number; showBuildVersion?: number; focusedFile?: string | null }) {
  ...
  return <section className="topology-views"><nav aria-label="Component graph lenses"><button aria-pressed={view === "service"} onClick={() => setView("service")}>Service</button><button aria-pressed={view === "build"} onClick={() => { setOpened(true); setView("build"); }}>Build graph</button></nav>
    ...
    {opened ? <div className="topology-view" hidden={view !== "build"}><BuildGraphPane focusedFile={focusedFile} visible={view === "build"} capture={capture} mockAgents={mockAgents} mockVersion={mockVersion} onOpenBuild={onOpenBuild} reframeVersion={view === "build" ? reframeVersion : 0} /></div> : null}
  </section>;
}

export function BuildGraphPane({ capture, mockAgents, mockVersion, onOpenBuild, reframeVersion = 0, focusedFile = null, visible = true }: { capture?: BuildLinkSnapshot; mockAgents: boolean; mockVersion: number; onOpenBuild: (path: string) => void; reframeVersion?: number; focusedFile?: string | null; visible?: boolean }) {
  ...
  const nodes = useMemo(() => {
    return [...layoutBuildTargets(slice?.depths ?? new Map())].map(([label, position]): Node =>
      ({ id: label, type: "buildTarget", position, draggable: false, data: { label, mock: sprites, ownsFile: fileSlice?.owners.includes(label) ?? false } }));
  ...
  return <section className="build-graph-view" aria-label="Bazel build graph" data-file-focus={followingFile ? focusedFile : undefined}><div className="build-target-controls"><form onSubmit={(event) => { event.preventDefault(); if (canAdd) { setFollowFile(false); setRoots((before) => [...new Set([...before, pattern])].slice(0, BUILD_PATTERN_LIMIT)); } }}>
    ...
    <div id="build-pattern-feedback" className="build-capture-caption" role="status">{!matches.length ? "No captured rule targets match. Use //..., //path/..., //path:all, //path:* or an exact label." : roots.includes(pattern) ? followingFile ? "Add target returns to your saved manual selection." : "Already in this view." : roots.length >= BUILD_PATTERN_LIMIT ? `Remove a pattern to add another (${BUILD_PATTERN_LIMIT} max).` : `${matches.length} captured rule target${matches.length === 1 ? "" : "s"} match${matches.length === 1 ? "es" : ""}.`}{matches.length > BUILD_VIEW_LIMIT ? ` Showing at most ${BUILD_VIEW_LIMIT}; narrow the package to see the rest.` : ""}</div>
    {followingFile ? <div className="build-file-focus"><strong>{focusedFile}</strong><span>{fileSlice?.owners.length ? `${fileSlice.owners.length} captured direct file target${fileSlice.owners.length === 1 ? "" : "s"} · dependents → file targets → dependencies` : "No captured target references this file. Ownership is unavailable—not inferred from its directory."}</span></div> : <div className="build-root-chips">{roots.map((root) => <button key={root} onClick={() => setRoots((before) => before.filter((label) => label !== root))} title="Remove target from this view">{root} ×</button>)}</div>}
    ...
    <div className="build-canvas"><ReactFlow onInit={(instance) => { flow.current = instance; }} onMoveStart={(event) => { if (event) cancelReframe(); }} nodes={nodes} edges={edges} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: .2, maxZoom: 1.2 }} minZoom={.05} nodesConnectable={false} onNodeClick={(_event, node) => setSelected(node.id)}><Background gap={22} color="#20352c" /><Controls showInteractive={false} /></ReactFlow>
      {nodes.length === 0 ? <p className="build-empty-hint">{followingFile ? "No captured build links for this file. Uncheck Follow file to explore targets manually." : "Add a captured target to explore its dependencies. No build is started."}</p> : null}
