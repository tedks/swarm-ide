...
export function ProjectContextPanel({ repositoryId, worldId, generation, ready }: Identity) {
  ...
  if (!observation || (!observation.servers.length && !observation.containers.length
    && !catalog?.components.length && !catalog?.relationships.length && !catalog?.sites.length)) return null;
  return <section className="project-runtime" aria-label="Project runtime">
    <header><span className="eyebrow">Project context</span><button type="button" aria-label="Refresh project runtime" disabled={!ready} onClick={() => setRefresh((value) => value + 1)}>↻</button></header>
    ...
    {observation.servers.length ? <><h3>Dev servers <Retained scan={observation.node} /></h3>
    {observation.servers.map((server) => <article key={server.pid}>
    ...
    {observation.node.status === "partial" ? <p className="project-runtime-empty">Some processes could not be inspected</p> : null}</> : null}
    {observation.containers.length ? <><h3>Containers <Retained scan={observation.docker} /></h3>
    {observation.containers.map((container) => <article key={container.id}>
    ...
    {observation.docker.status === "partial" ? <p className="project-runtime-empty">Some container details unavailable</p> : null}</> : null}
    <ProjectCatalogPanel catalog={catalog} />
