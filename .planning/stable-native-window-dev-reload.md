...
Hosted PR #7 code run 33999215499 passed build and unit tests but reproduced that exact 30-second topology wait failure. A matching local run with the source workspace explicitly set passed in 26.8 seconds. The harness gives the app an isolated XDG cache, so the initially suspected shared Bazel lock was not reproduced. Increase only the Consistent-title deadline to 90 seconds, within the existing 120-second scenario bound; keep all state assertions and require hosted success. This narrow calibration addresses the recorded debt without an independent CI/resource redesign.
