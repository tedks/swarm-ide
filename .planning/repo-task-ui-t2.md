...
Native convergence on `d3ec377` was CLEAN; that topic passed full25/all9 and 808 tests. Its second task-specific virtual run was cancelled while only this step's flock process was waiting, before any GUI launched, because Anthropic's completed review added further corrections. The first failed virtual evidence is retained honestly. Anthropic independently confirmed two initial findings and found keyboard focus loss after Reveal/Return, failed background tabs surviving in desired navigation, failure notices being erased by pending cheap checks, and cross-kind sequence ordering assumptions. These are addressed before the next full proof: explicit Reveal hands focus to the editor without moving a dirty cursor, typed failures focus their notice, Return focuses the source-information heading, rejected empty background tabs are unwatched/removed, and task snapshot/detail streams use separate watermarks with their existing pinned revision/selection guards. Failure notice remains until valid observation; reasons are no longer duplicated.

Review follow-ups are filed as `repo-task-ui-followups`: pre-existing CRLF source-flash offset normalization, conservative Work-only compact freshness policy, dead dispatch CSS outside the owned style scope, and optional extraction of the idempotent editor-memory getter. CRLF task descriptions now display normal newlines while lone controls remain escaped. Task client recreation follows the existing agent-client Fast Refresh pattern; initial full-scan transport failure conservatively requires explicit full Refresh rather than automatic scan retries.

Quota-resumed review exposed a keyboard race introduced by explicit asynchronous handoffs: entering an agent draft or Ctrl-K while Reveal waits did not revoke the old intent. A pending-only token is now revoked by newer cockpit pointer/focus interactions or the shortcut itself; finally clears only its own token before normal explicit handoffs. Four delayed success/failure tests cover draft/palette destinations. The first test run exposed a test-harness error: fireEvent.click did not move focus from the auto-focused draft, so the helper now focuses the initiating Reveal button as an actual user click would. The second run passed. No production workaround for that synthetic input behavior was added.
