...
- Decision: capture Git's tracked plus non-ignored untracked names (including dotfiles) using fixed read-only `ls-files --cached --others --exclude-standard -z`, with the existing sanitized Git runner. A separate index-only `ls-files --stage -z` retains gitlink exclusions; both calls share the 2-second / 2-MiB capture budget. Tracked names still appear when ignore rules match. Ignore configuration is data; no fetch/hooks/write/filter command runs. Symlinks, gitlinks, nested repositories, special files and unsupported filename bytes are not actionable matches. Separate streams avoid ambiguous stage-looking untracked names without losing gitlinks that were replaced on disk by regular files. Rationale: reuse Git's known boundaries rather than recursively walk the repository on every key. Date: 2026-09-07, N2.
...
The 782b298 rerun passed 12/13 suites, including packaged search and tasks, but the older topology desktop harness also dropped a character during its 2ms native exact-path input: its saved screenshot reports `fraudcheck.s`, not the requested `fraudcheck.ts`. Apply the same precise mode-transition wait and 10ms human-paced input to that harness; retain all source, topology and graphical assertions. The failed run is not a passing aggregate. This is a bounded harness mitigation, not proof that the loss originated in synthetic delivery rather than product input handling. If loss recurs after exact-mode readiness at 10ms, investigate input handling instead of increasing delays again. Final full verification is required after this correction.
