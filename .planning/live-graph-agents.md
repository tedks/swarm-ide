...
ROOT accepted this substance at 897f837 and explicitly kept the original GUI close/pointer limitations as noncritical follow-ups. Its landing preparation merges only reviewed master b20c69f0. The sole manual import conflict retains both plan-generation controls and graph-agent helpers; automatic App generation/tab and build-tab ordering changes remain. The focused check now also covers plan-generation UI and tab-order/overflow seams. Both types and 37 tests pass, while living-design fails at module load on the separately assigned canonical plan buildTargets maximum16 boundary. The later fresh-startup invocation therefore did not run in this composition. No mappings were removed, plan schema repaired, unchanged test retried, GUI/model trial performed or camera scope added. ROOT clears the separate plan fix before composed adoption as needed; this branch provides a reviewed join, not a full-green claim.
