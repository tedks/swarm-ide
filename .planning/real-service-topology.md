...
- Observation: `bazel-bin` is a workspace convenience symlink into Bazel's external output tree and is not authenticated merely by resolving it.
  Evidence: the provider now asks Bazel for its absolute output directory and independently compares the artifact's versioned input digest with canonical current files before publication.
