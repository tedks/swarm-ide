...
- [x] (2026-09-05 20:35Z) Addressed the first council findings on build-output provenance, compiled unary RPC validation, whole-workspace observation, async file fences, and conflict-safe renderer state.
- [x] (2026-09-05 20:40Z) Refined the cockpit into persistent graph/source/instrument columns, left-to-right service flow, selectable relationship instruments, contained labels, and renderer-owned Ctrl-W tab closing.
- [x] (2026-09-05 20:55Z) Closed council race/accessibility findings, made idle yellow graph dots build actions, and passed 19 Bazel-owned files / 102 tests.
- [x] (2026-09-05 21:18Z) Addressed convergence findings by composing batched tab closes, serializing initial file observation, accepting delayed own-save events, and preserving the exact Bazel-reported path for no-follow artifact reads; passed 19 Bazel-owned files / 104 tests.
- [x] (2026-09-05 21:28Z) Closed the remaining source-observation handoff and reverse-order active-tab close races with explicit generation/surface authority; passed 19 Bazel-owned files / 105 tests.
...
- Observation: `bazel-bin` is a workspace convenience symlink into Bazel's external output tree and is not authenticated merely by resolving it or by a later configuration lookup.
  Evidence: the provider accepts exactly one local artifact named by the successful target-completion event of the exact build invocation, then independently compares its versioned input digest with canonical current files before publication.
- Observation: tab-scoped filesystem watches cannot establish global working-world consistency because unopened, new, BUILD, or manifest files can change.
  Evidence: the core now uses one globally ordered Git observer with a bounded polling fallback; tests prove stale failures and cross-request computations cannot publish after a newer observation.
...
- Decision: carry file reads and writes as dedicated runtime-validated core requests and responses through the existing utility process.
  ...
  Date/Author: 2026-09-05 / Integration Architect.
- Decision: preserve graph instances while file tabs are active and keep them visible as the source observatory's navigation column.
  Rationale: React Flow camera state remains local to each graph instance and survives graph-to-file transitions without new public presentation state. After user steering, those live instances remain visible as the navigation column rather than being visually hidden.
  Date/Author: 2026-09-05 / Integration Architect.
...
- Decision: use public protobuf/gRPC-style contracts as the demo analogue of Stubby, but keep topology identity in the small `service.swarm.json` declaration and implementation ownership in Bazel.
  ...
  Date/Author: 2026-09-05 / User steering and Integration Architect.
- Decision: treat an edge selection as contextual relationship focus derived from its bounded graph slice, not as a new protocol-domain artifact.
  Rationale: the user can inspect direction, endpoints, interface contract, source, and provenance while the separate repo/service graph ontology remains intact.
  Date/Author: 2026-09-05 / User steering and Integration Architect.

## Outcomes & Retrospective

The implementation gate is satisfied pending council/CI/merge. Production now opens the actual `swarm-ide` working tree with an empty gray service graph, no fixture claims, and an explicit `not configured` deployment. The exact target `//examples/checkout-world/services/fraudcheck:service_topology` produces one semantic artifact at `bazel-bin/examples/checkout-world/services/fraudcheck/service-topology.json`; the current deterministic artifact has SHA-256 `d8801cb1b69f61750f663b984eb78f84b00eb4d82c285d7556cbca42cdf86f33` and canonical-input digest `dbdaf15e1acef3fa6bcd091a998311157a834b1f87053e10371d83d297c74429`.

The real provider captures Git HEAD plus changed/deleted/non-ignored untracked paths, modes, and bytes. It publishes yellow for that working fingerprint, invokes Bazel with fixed arguments and no shell, resolves exactly one bounded artifact from that invocation's build-event stream, recomputes the fingerprint, and uses the artifact SHA-256 as build ID. An exact mismatch, failed build, malformed or ambiguous event/artifact, duplicate request, or older attempt cannot publish green; red retains the previous graph. Polling never invalidates its own slower in-flight computation, and a successful observation can clear only a red state caused by observation failure.

The source observatory opens canonical bounded UTF-8 files through the typed bridge. It rejects traversal, symlink escape, non-files, binary/control bytes, invalid Unicode, oversize content, and stale expected revisions. Saves use a unique sibling, preserve mode, fsync, revalidate immediately before atomic rename, and return the new working fingerprint. External clean-buffer changes render additions green and removals as a temporary red ghost; dirty buffers become conflicts without replacement. The same graphs stay visible beside source, and Ctrl-W cannot close the application.

Validation evidence: `bazel test //...` and `bazel build //...` pass; the Bazel-owned quality target reports 19 files and 105 tests passing. Five final warm runs measured click-to-yellow 67/68/112/68/68 ms (median 68 ms; one run missed the 100 ms target) and click-to-green 963/1002/1048/957/956 ms (median 963 ms, observed p95/max 1048 ms), within the green target; an earlier cold post-change build exceeded the smoke wait while Bazel rebuilt dependencies. A separate real-window click on the new unlabeled yellow graph dot launched the build and reached green in 965 ms; its 116 ms click-to-yellow observation missed the aspirational 100 ms bound. X11 screenshots under `artifacts/real-topology/`, `artifacts/desktop-final/`, and `artifacts/cockpit-final/` show gray→yellow→green, the real graphs/contracts, relationship inspection, graph↔file tabs, live green insertion/red deletion, and preserved conflict content. Real-window Ctrl-S wrote the expected probe and changed the working world; a subsequent external restore left the unsaved buffer visible under `CONFLICT`. Ctrl-W closed source tabs through the renderer and was a no-op on the graph surface while Electron remained alive. Final HMR measured 70 ms edit-to-pixel, 99 ms edit-to-title, and 11 ms Vite-event-to-paint on port 55174.

Remaining risk is intentionally narrow: POSIX/Node does not offer an atomic compare-and-swap pathname operation against uncooperative external writers, so saves revalidate immediately before atomic replacement and the renderer preserves any observed concurrent conflict, but a writer racing inside that final syscall boundary still needs a future cooperative write broker or native transaction helper. Source flashes use one bounded line-level replacement region rather than a full semantic diff. Agent attribution, durable change history, generalized provider discovery, LSP, terminals, and merge UI remain follow-up scope.
