// TEST ONLY. Real disposable Git/Bazel input; no mocked core or renderer data.
...
const owner = process.env.SWARM_X11_OWNERSHIP_DIR, evidence = await realpath(process.env.SWARM_CONTEXT_EVIDENCE);
...
await access(bundle);
const scratch = await mkdtemp(join(owner, "context-metrics-package-"));
...
try {
  ...
  fixture.sourceText = "// @swarm-demo-latency checkout.assess\nexport const example = true;\n";
  await writeFile(join(fixture.root, fixture.sourcePath), fixture.sourceText);
  await writeFile(join(fixture.root, "README.md"), "# Untargeted document\nNo telemetry tag here.\n");
  await writeFile(join(evidence, "fixture.json"), JSON.stringify(fixture));
  server = createServer((_request, response) => { response.writeHead(200); response.end("owned packaged Context proof"); });
