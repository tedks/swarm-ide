...
if (!proof.ok || !proof.packagedCore || !proof.retained || !proof.keyboard ||
    proof.unexpectedMutations.length || proof.startupBuildRequests.length > 1 || proof.resourceRequests.length || proof.rendererErrors.length) process.exit(1);
console.log('Packaged resource example: keyboard controls, real unsaved source and graph retention; no requests from resource controls.');
console.log('Existing packaged startup build requests (not resource actions):', proof.startupBuildRequests.length);
JS
