...
import { createNavigationFixture } from "./fixture.mjs";
...
const evidence = await realpath(process.env.SWARM_NAVIGATION_EVIDENCE);
if (Number(process.env.SWARM_DEV_PORT) !== 55174) throw new Error("Navigation proof requires owned test port 55174");
...
await access(bundle);
const scratch = await mkdtemp(join(owner, "swarm-navigation-package-"));
...
try {
  ...
  const fixture = await createNavigationFixture(scratch, process.env.SWARM_NAVIGATION_CASE, process.cwd());
  await writeFile(join(evidence, "fixture.json"), JSON.stringify(fixture));
  server = createServer((_request, response) => { response.writeHead(200); response.end("owned packaged navigation proof"); });
  await new Promise((resolve, reject) => { server.once("error", reject); server.listen(55174, "127.0.0.1", resolve); });
  const environment = { ...process.env, NODE_PATH: "", SWARM_NAVIGATION_PACKAGE: extracted, SWARM_NAVIGATION_PROFILE: profile };
