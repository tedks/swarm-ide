...
const evidence = await realpath(process.env.SWARM_SERVICE_EVIDENCE);
...
await access(bundle);
const scratch = await mkdtemp(join(owner, "services-"));
let server, desktop, killTimer;
const handlers = new Map();
try {
  const extracted = join(scratch, "app"), profile = join(scratch, "profile"), root = join(scratch, "declared-project");
  ...
  await writeFile(join(root, "README.md"), "# Independent service project\n\nThis source remains open while declarations refresh.\n");
  await writeFile(join(root, "compose.yaml"), "services:\n  frontend:\n    image: local/frontend\n    depends_on: [backend]\n  backend:\n    image: local/backend\n");
  execFileSync("git", ["init", "-q"], { cwd: root });
  execFileSync("git", ["add", "."], { cwd: root });
  execFileSync("git", ["-c", "user.name=Proof", "-c", "user.email=proof@example.invalid", "commit", "-qm", "disposable declarations"], { cwd: root });
  server = createServer((_request, response) => { response.writeHead(200); response.end("owned service proof"); });
  await new Promise((resolve, reject) => { server.once("error", reject); server.listen(port, "127.0.0.1", resolve); });
  const environment = { ...process.env, NODE_PATH: "", SWARM_SERVICE_PACKAGE: extracted, SWARM_SERVICE_FIXTURE: root };
