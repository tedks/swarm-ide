...
import { readFileSync, writeFileSync, mkdirSync, renameSync, existsSync, readdirSync, readlinkSync } from 'node:fs';
import { spawn, spawnSync } from 'node:child_process';
import { connect } from 'node:net';
import { ENV, REQUIRED, summarizePages, LIMIT } from './boundary.mjs';
...
async function boundary() {
  ...
  checks.noHostDescriptors = readdirSync('/proc/self/fd').every(fd => {
    ...
    catch { return true; } // readdir's own descriptor is closed before this walk.
  });
...
async function inspect() {
  ...
  const pending = new Map(); let id = 0, bytes = 0, buffer = '', fault = null;
  ...
  server.stderr.on('data', chunk => { bytes += chunk.length; if (bytes > LIMIT) fail('SERVER_OUTPUT_LIMIT'); });
...
try {
  if (mode === 'boundary') emit({ checks: await boundary(), namespace: readlinkSync('/proc/self/ns/pid') });
  else if (mode === 'hold') {
    ...
    setInterval(() => {}, 1000);
  } else if (mode === 'flood') { for (;;) process.stdout.write('x'.repeat(4096)); }
